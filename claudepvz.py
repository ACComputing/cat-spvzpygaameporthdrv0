...
# ═══════════════════════════════════════════════════════════════
#  AUDIO ENGINE — PVZ1-Faithful Procedural SFX & Music
# ═══════════════════════════════════════════════════════════════
...
def gen_noise(dur_s, vol=0.1):
    n = int(SAMPLE_RATE * dur_s)
    return [random.uniform(-1, 1) * vol for _ in range(n)]

def _mix(*lists):
    ml = max(len(s) for s in lists)
    out = [0.0] * ml
    for s in lists:
...
# ── SFX (PVZ1-Faithful Procedural Sound Effects) ──
class SFX:
    """Procedural SFX approximating PVZ1 audio — no external files."""
    _cache = {}

    @staticmethod
    def _make(key, gen_func):
        if key not in SFX._cache:
            SFX._cache[key] = gen_func()
        return SFX._cache[key]

    @staticmethod
    def plant():
        def gen():
            return samples_to_sound(
                gen_note(580, 0.04, 0.35, "sine", 0.003, 0.02, 0.2, 0.02) +
                gen_note(420, 0.06, 0.3, "sine", 0.003, 0.03, 0.15, 0.03) +
                gen_note(280, 0.05, 0.2, "triangle", 0.005, 0.02, 0.1, 0.02), 0.7)
        SFX._make("plant", gen).play()

    @staticmethod
    def sun():
        def gen():
            return samples_to_sound(
                gen_note(880, 0.05, 0.3, "sine", 0.005, 0.02, 0.4, 0.02) +
                gen_note(1100, 0.05, 0.25, "sine", 0.005, 0.02, 0.3, 0.02) +
                gen_note(1320, 0.07, 0.2, "sine", 0.005, 0.03, 0.2, 0.03), 0.7)
        SFX._make("sun", gen).play()

    @staticmethod
    def shoot():
        def gen():
            return samples_to_sound(
                gen_note(500, 0.025, 0.2, "sine", 0.002, 0.01, 0.15, 0.008) +
                gen_note(380, 0.02, 0.12, "triangle", 0.002, 0.008, 0.1, 0.006), 0.5)
        SFX._make("shoot", gen).play()

    @staticmethod
    def chomp():
        def gen():
            return samples_to_sound(_mix(
                gen_note(180, 0.08, 0.3, "square", 0.005, 0.03, 0.3, 0.03),
                gen_note(120, 0.08, 0.2, "saw", 0.005, 0.04, 0.2, 0.03),
                gen_noise(0.06, 0.15) + [0] * int(SAMPLE_RATE * 0.02)), 0.6)
        SFX._make("chomp", gen).play()

    @staticmethod
    def explode():
        def gen():
            return samples_to_sound(_mix(
                gen_note(60, 0.2, 0.4, "sine", 0.005, 0.08, 0.3, 0.1),
                gen_note(150, 0.08, 0.3, "saw", 0.002, 0.03, 0.2, 0.04) + [0] * int(SAMPLE_RATE * 0.12),
                gen_note(40, 0.3, 0.25, "triangle", 0.01, 0.1, 0.2, 0.15),
                gen_noise(0.15, 0.2) + [0] * int(SAMPLE_RATE * 0.15)), 0.8)
        SFX._make("explode", gen).play()

    @staticmethod
    def mow():
        def gen():
            return samples_to_sound(_mix(
                gen_note(100, 0.25, 0.3, "saw", 0.02, 0.05, 0.4, 0.15),
                gen_note(160, 0.15, 0.2, "square", 0.03, 0.05, 0.3, 0.05) + [0] * int(SAMPLE_RATE * 0.1),
                gen_note(80, 0.3, 0.15, "saw", 0.01, 0.05, 0.3, 0.1, 15, 20)), 0.7)
        SFX._make("mow", gen).play()

    @staticmethod
    def click():
        def gen():
            return samples_to_sound(
                gen_note(1200, 0.02, 0.2, "sine", 0.001, 0.008, 0.1, 0.008), 0.5)
        SFX._make("click", gen).play()
    ...
    def win():
        def gen():
            ...
            for f in [523, 659, 784, 1047]:
                n = gen_note(f, 0.12, 0.3, "sine", 0.008, 0.03, 0.5, 0.04)
                h = gen_note(f * 2, 0.12, 0.1, "sine", 0.008, 0.03, 0.3, 0.04)
                s.extend([a + b for a, b in zip(n, h)])
            s.extend(gen_note(1047, 0.3, 0.35, "sine", 0.01, 0.05, 0.5, 0.15, 5, 3))
            return samples_to_sound(s, 0.7)
        SFX._make("win", gen).play()
    ...
    def lose():
        def gen():
            ...
            for f in [392, 330, 262, 196]:
                s.extend(gen_note(f, 0.2, 0.3, "saw", 0.01, 0.05, 0.4, 0.08, 5, 3))
            s.extend(gen_note(165, 0.4, 0.3, "saw", 0.01, 0.08, 0.3, 0.2, 3, 5))
            return samples_to_sound(s, 0.7)
        SFX._make("lose", gen).play()

    @staticmethod
    def shovel():
        def gen():
            return samples_to_sound(_mix(
                gen_note(300, 0.06, 0.25, "saw", 0.005, 0.02, 0.2, 0.03),
                gen_noise(0.05, 0.15) + [0] * int(SAMPLE_RATE * 0.01)), 0.6)
        SFX._make("shovel", gen).play()
    ...
    def splash():
        def gen():
            return samples_to_sound(_mix(
                gen_noise(0.15, 0.25),
                gen_note(200, 0.1, 0.15, "sine", 0.005, 0.04, 0.1, 0.06) + [0] * int(SAMPLE_RATE * 0.05)), 0.6)
        SFX._make("splash", gen).play()

    @staticmethod
    def frozen():
        def gen():
            return samples_to_sound(_mix(
                gen_note(2000, 0.08, 0.15, "sine", 0.003, 0.03, 0.15, 0.04),
                gen_note(1500, 0.08, 0.1, "triangle", 0.005, 0.04, 0.1, 0.03),
                gen_note(3000, 0.05, 0.06, "sine", 0.002, 0.02, 0.05, 0.02) + [0] * int(SAMPLE_RATE * 0.03)), 0.5)
        SFX._make("frozen", gen).play()

    @staticmethod
    def gulp():
        def gen():
            return samples_to_sound(
                gen_note(400, 0.08, 0.25, "sine", 0.005, 0.04, 0.15, 0.03) +
                gen_note(200, 0.07, 0.2, "sine", 0.01, 0.03, 0.1, 0.03), 0.6)
        SFX._make("gulp", gen).play()

    @staticmethod
    def zombie_groan():
        def gen():
            return samples_to_sound(
                gen_note(120, 0.25, 0.2, "saw", 0.03, 0.08, 0.25, 0.1, 3, 8), 0.4)
        SFX._make("groan", gen).play()

    @staticmethod
    def seed_select():
        def gen():
            return samples_to_sound(
                gen_note(700, 0.025, 0.2, "sine", 0.002, 0.01, 0.15, 0.008) +
                gen_note(900, 0.02, 0.12, "sine", 0.002, 0.008, 0.1, 0.008), 0.5)
        SFX._make("seed_sel", gen).play()
    ...
    def wave_siren():
        def gen():
            s = []
            for i in range(3):
                s.extend(gen_note(600 + i * 100, 0.08, 0.25, "sine", 0.01, 0.03, 0.3, 0.03))
                s.extend([0] * int(SAMPLE_RATE * 0.04))
            return samples_to_sound(s, 0.6)
        SFX._make("wave_siren", gen).play()
    ...
    def pop():
        def gen():
            return samples_to_sound(_mix(
                gen_noise(0.03, 0.3),
                gen_note(800, 0.02, 0.2, "sine", 0.001, 0.008, 0.1, 0.008) + [0] * int(SAMPLE_RATE * 0.01)), 0.6)
        SFX._make("pop", gen).play()

    @staticmethod
    def buzzer():
        def gen():
            return samples_to_sound(
                gen_note(200, 0.15, 0.25, "square", 0.005, 0.03, 0.3, 0.06), 0.5)
        SFX._make("buzzer", gen).play()

# ═══════════════════════════════════════════════════════════════
#  MUSIC ENGINE — PVZ1-Style Procedural Loops (Background Gen)
# ═══════════════════════════════════════════════════════════════
class MusicEngine:
    """Generates and plays PVZ1-style procedural music loops per world."""
    THEMES = {
        "day": {"tempo": 130, "wave": "sine", "vol": 0.2, "melody": [
            (72,1),(76,1),(79,1),(76,1),(77,1),(81,1),(79,1),(76,1),
            (74,1),(77,1),(76,1),(72,1),(74,1),(76,1),(74,1),(72,2),
            (72,1),(76,1),(79,2),(77,1),(76,1),(74,1),(72,1),
            (74,1),(76,1),(72,2),(71,1),(72,2),(0,1)],
            "bass": [(48,2),(48,2),(53,2),(53,2),(50,2),(50,2),(48,2),(48,2),
                     (48,2),(48,2),(53,2),(53,2),(50,2),(50,2),(48,4)]},
        "night": {"tempo": 100, "wave": "triangle", "vol": 0.15, "melody": [
            (69,2),(72,2),(76,2),(69,2),(67,2),(71,2),(74,2),(67,2),
            (65,2),(69,2),(72,2),(65,2),(64,2),(67,2),(71,2),(64,2)],
            "bass": [(45,4),(45,4),(43,4),(43,4),(41,4),(41,4),(40,4),(40,4)]},
        "pool": {"tempo": 140, "wave": "sine", "vol": 0.18, "melody": [
            (74,1),(78,1),(81,1),(86,1),(83,1),(81,1),(78,1),(74,1),
            (76,1),(79,1),(83,1),(76,1),(81,1),(78,1),(74,1),(69,2),
            (74,1),(78,1),(81,2),(83,1),(81,1),(78,1),(74,1),
            (76,1),(74,1),(71,2),(74,2),(0,2)],
            "bass": [(50,2),(50,2),(55,2),(55,2),(52,2),(52,2),(50,2),(50,2),
                     (50,2),(50,2),(55,2),(55,2),(52,2),(52,2),(50,4)]},
        "fog": {"tempo": 90, "wave": "triangle", "vol": 0.12, "melody": [
            (58,3),(61,3),(65,2),(56,3),(60,3),(63,2),
            (54,3),(58,3),(61,2),(53,3),(56,3),(60,2)],
            "bass": [(34,4),(34,4),(32,4),(32,4),(30,4),(30,4),(29,4),(29,4)]},
        "roof": {"tempo": 150, "wave": "square", "vol": 0.15, "melody": [
            (79,1),(83,1),(86,1),(79,1),(81,1),(84,1),(88,1),(81,1),
            (78,1),(81,1),(86,1),(78,1),(79,1),(83,1),(86,1),(83,1),
            (79,1),(83,1),(86,2),(84,1),(81,1),(79,1),(76,1),
            (79,2),(0,2),(78,1),(79,2),(0,1)],
            "bass": [(43,2),(43,2),(45,2),(45,2),(42,2),(42,2),(43,2),(43,2),
                     (43,2),(43,2),(45,2),(45,2),(42,2),(42,2),(43,4)]},
        "menu": {"tempo": 120, "wave": "sine", "vol": 0.18, "melody": [
            (65,1),(69,1),(72,2),(74,1),(72,1),(69,1),(65,1),
            (67,1),(72,1),(74,2),(72,1),(69,1),(67,2),
            (65,1),(69,1),(72,2),(74,1),(77,1),(76,2),
            (74,1),(72,1),(69,1),(65,1),(67,1),(65,2),(0,1)],
            "bass": [(41,2),(41,2),(48,2),(48,2),(43,2),(43,2),(41,2),(41,2),
                     (41,2),(41,2),(48,2),(48,2),(43,2),(43,2),(41,4)]},
    }
    ...
    def __init__(self):
        ...
        print("  Music: ON (generating PVZ1-style procedural OST...)")
        self._gen_thread = threading.Thread(target=self._generate_all, daemon=True)
        self._gen_thread.start()

    def _gen_theme(self, name):
        theme = self.THEMES[name]
        tempo, wave, vol = theme["tempo"], theme["wave"], theme["vol"]
        bd = 60.0 / tempo
        mel = []
        for note, beats in theme["melody"]:
            d = bd * beats
            if note == 0:
                mel.extend([0.0] * int(SAMPLE_RATE * d))
            else:
                f = midi_to_freq(note)
                s = gen_note(f, d, vol, wave, 0.01, 0.05, 0.5, 0.05,
                             4 if wave == "triangle" else 0, 2)
                h = gen_note(f * 2, d, vol * 0.2, "sine", 0.015, 0.05, 0.3, 0.05)
                mel.extend([a + b for a, b in zip(s, h)])
        bas = []
        for note, beats in theme["bass"]:
            d = bd * beats
            if note == 0:
                bas.extend([0.0] * int(SAMPLE_RATE * d))
            else:
                bas.extend(gen_note(midi_to_freq(note), d, vol * 0.6, "triangle",
                                    0.02, 0.08, 0.4, 0.08))
        ml = max(len(mel), len(bas))
        mel.extend([0.0] * (ml - len(mel)))
        bas.extend([0.0] * (ml - len(bas)))
        return samples_to_sound([m + b for m, b in zip(mel, bas)], 0.6)

    def _generate_all(self):
        self._generating = True
        for name in self.THEMES:
            try:
                self._sounds[name] = self._gen_theme(name)
            except Exception as e:
                print(f"  Music gen error ({name}): {e}")
        try:
            s = []
            for f in [523, 659, 784, 1047]:
                n = gen_note(f, 0.12, 0.3, "sine", 0.008, 0.03, 0.5, 0.04)
                ...
                s.extend([a + b for a, b in zip(n, h)])
            s.extend(gen_note(1047, 0.3, 0.35, "sine", 0.01, 0.05, 0.5, 0.15, 5, 3))
            self._sounds["win"] = samples_to_sound(s, 0.7)
        except:
            pass
        try:
            s = []
            for f in [392, 330, 262, 196]:
                s.extend(gen_note(f, 0.2, 0.25, "saw", 0.01, 0.05, 0.4, 0.08, 5, 3))
            s.extend(gen_note(165, 0.4, 0.3, "saw", 0.01, 0.08, 0.3, 0.2, 3, 5))
            self._sounds["lose"] = samples_to_sound(s, 0.6)
        except:
            pass
        self._generating = False
        print("  Music: All themes generated!")

    def play(self, theme_name):
        if not self.enabled or theme_name == self.current_theme:
            return
        if theme_name not in self._sounds:
            return
        self.stop()
        self.current_theme = theme_name
        snd = self._sounds[theme_name]
        snd.set_volume(self.volume)
        if theme_name in ("win", "lose"):
    ...
    def set_volume(self, vol):
        ...
        if self.channel and self.current_theme in self._sounds:
            try:
                self._sounds[self.current_theme].set_volume(self.volume)
            except:
                pass
    ...
    def update_for_state(self, game_state, level_type=None):
        if not self.enabled:
            return
        if game_state == "GAME":
            theme = level_type or "day"
            if theme in self._sounds and self.current_theme != theme:
                self.play(theme)
        elif game_state in ("MENU", "SEED_SELECT", "SHOP", "ALMANAC", "ZEN_GARDEN",
                            "MINIGAME_SELECT", "SURVIVAL_PICK", "VS_SETUP"):
            if self.current_theme != "menu":
                self.play("menu")
    ...
    def play_result(self, won):
        if not self.enabled:
            return
        self.stop()
        theme = "win" if won else "lose"
        if theme in self._sounds:
            self.current_theme = theme
            snd = self._sounds[theme]
            snd.set_volume(self.volume)
            self.channel = snd.play()
